...
* Views library: set of powerful views, typically operating and presenting
  server metadata
* Routines library: set of complementary routines to MySQL, including metadata
  handling
* QueryScript: programming language aimed for SQL scripting, seamlessly
...
rdebug provides with an API that can be used by all clients - from command
line sessions to GUI tools. The API is based on stored routine calls.
rdebug works by running two MySQL sessions: the worker session, where one
issues her stored routine, and the debugger session, which attaches to the
...
See rdebug_workflow to find out how to set up and work out a debugging
session.
rdebug is in alpha - experimental - stage. It provides with working
...
* rdebug_run() is known to hang when the entire routine executes without at
...
rdebug is in alpha stage, and API or functionality are still subjected to
change.
...
* rdebug_step_into(): Enter next routine call (requires routine in debug
...
* rdebug_show_routine_statements(): Show statement (breakpoint) IDs for a
  given routine.
...
This procedure is part of the rdebug_API.
rdebug_compile_routine() modifies a routine code to include/exclude debug
info.
rdebug works by injecting debug code into stored routines. In order to debug a
...
* debug_info: TRUE or 1 to inject debug code into the routine, FALSE or 0 to
...
This function is part of the rdebug_API.
...
Variables which are visible are those presented by rdebug_watch_variables().
...
suspended on a breakpoint or by a step (into/out/over). During that time it is
...
similarly to rdebug_watch_variables(). However, it does not return any
metadata for the variable.
Execution of this routine only makes sense on an active debugging session,
i.e. after calling rdebug_start().
...
* rdebug_variable_name: name of local variable or user defined variable. User
...
rdebug_set_variable(), rdebug_watch_variables()
...
As opposed to rdebug_step_into() or rdebug_step_over(), where the worker is
advanced one "step", rdebug_run() allows the worker to run freely until it
reaches an active breakpoint - such that has been set by rdebug_set_breakpoint
(). If not such breakpoint is encountered, the worker is executed to full
...
rdebug_set_breakpoint(), rdebug_start(), rdebug_step_into(), rdebug_step_out
(), rdebug_step_over()
...
rdebug_show_routine() or rdebug_show_routine_statements().
A breakpoint may be conditional [NOTE: as yet unsupported], such that it only
applies when some condition evaluates as true (e.g. ''my_loop_counter > 3'').
Current implementation [NOTE: subject to change] associates breakpoints with
...
rdebug_start() [NOTE: subject to change].
...
* rdebug_conditional_expression: AS YET UNSUPPORTED - expression to evaluate
  upon reaching breakpoint. The breakpoint holds only when the expression
  evaluates to TRUE value; otherwise the breakpoint is skipped by such
  commands as rdebug_run().
* breakpoint_enabled: a TRUE/FALSE or 1/0 value to indicate whether the
...
rdebug_watch_variables(). These are the local variables in scope at the
...
execution.
The new value is passed as BLOB and is later translated to the appropriate
...
* rdebug_variable_name: name of local variable or user defined variable to
...
worker resumes execution, it updates the value of current_continent to
''Atlantis''.
...
rdebug_set_verbose() causes an implicit invocation of rdebug_verbose()
...
* verbose: a TRUE or FALSE (or numeric equivalent) value to indicate requested
  verbose mode.
...
This procedure is part of the rdebug_API.
rdebug_show_routine() displays the body of a routine in prettified format. It
adds breakpoint positions/IDs to the display.
A breakpoint is shown just at the beginning of the statement following it.
Thus, setting_a_breakpoint implies the worker will stop just before executing
said statement.
...
In the above routine there are 9 breakpoint positions; these can be used as
input for rdebug_set_breakpoint().
...
routine. These IDs are eligible for rdebug_set_breakpoint(), and are presented
in rdebug_show_routine() and rdebug_show_stack_state().
Execution of this routine is possible at any given time. When the specified
routine has no debug info, rdebug_show_routine() simply shows its code.
...
rdebug_compile_routine(), rdebug_set_breakpoint(), rdebug_show_routine(),
...
In formation only applies to routines compiled_with_debug_mode. Any routine
...
In the above, analyze_continents() was invoked at 2013-03-30 16:52:13. At
2013-03-30 16:52:16, and at statement ID 127 it called upon
analyze_continent_cities(), currently waiting on statement ID 93.
...
the next statement to be executed by such action as rdebug_step_into(),
rdebug_step_over(), rdebug_run(), etc. rdebug_show_statement() presents with
routine schema & name, the statement ID (see rdebug_show_routine_statements())
and the statement code (see rdebug_show_routine()).
...
In the above, fetch continents_cursor into current_continent is the next
statement to execute. It has statement ID 98. Compare with output of
rdebug_show_routine_statements() and rdebug_show_routine().
...
One should call upon rdebug_start() after compiling_a_routine_or_more_with
debug_mode, and opening a worker connection.
...
* recipient_id: ID of the session we wish to debug; the worker, which receives
...
rdebug_compile_routine(), rdebug_run(), rdebug_step_over(), rdebug_stop(),
rdebug_workflow
...
itself compiled_with_debug_mode. It is possible that said routine is not
...
rdebug_compile_routine(), rdebug_run(), rdebug_start(), rdebug_step_out(),
rdebug_step_over()
...
first debugged routine -- execution is similar to that of rdebug_run().
Execution of this routine only makes sense on an active debugging session,
i.e. after calling rdebug_start().
...
rdebug_compile_routine(), rdebug_run(), rdebug_start(), rdebug_step_into(),
rdebug_step_over()
...
code, invoke rdebug_compile_routine().
...
rdebug_compile_routine(), rdebug_start(), rdebug_workflow
...
rdebug_set_verbose(), rdebug_show_stack_state(), rdebug_show_statement(),
rdebug_watch_variables()
...
excludes variables injected by rdebug itself, as these only serve to the
...
* variable_type: type of variable. Either:
  ...
  o param: routine parameter
  o local: local routine variable
  o user_defined: user defined (session) variable
...
* The debugger session: this session controls the flow of the worker; this is
...
out the worker''s session ID. The relevant routines must be compiled_with_debug
mode beforehand. Both sessions are then ready for the debugging session.
...
debugging the routine test.take_action().
...
Optionally set verbose mode on:
...
debugger session: at this time the debugger has control over the worker. Due
...
At this workflow we finally exit via rdebug_step_out(), or we may choose to
keep running rdebug_step_over() until test.take_action() completes. Either
...
One may choose to set a breakpoint on test.take_action(). For that, one needs
to know what breakpoints are available. debugger session: detect breakpoint
info:
...
See rdebug_show_routine() for an output sample. Breakpoints can be set via:
...
(assuming 126 is a relevant breakpoint ID).
The test.take_action() routine can be executed until it reaches said
breakpoint:
...
The replacement text may be constant, but may also use the ''\\\\0'' back-
...
* replacement_text: text to inject in place of sections. May contain the
  special ''\\\\0'' back-reference
* .
